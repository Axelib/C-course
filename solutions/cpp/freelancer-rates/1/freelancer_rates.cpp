...
double apply_discount(double before_discount, double discount) {
    ...
    double descprice=before_discount*(discount/100);
    return before_discount-descprice ;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double hourxmonth=176;
    double total=hourly_rate*hourxmonth;
    double discounted = apply_discount(total, discount);
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double daily_rate = hourly_rate * 8 * (1 - discount / 100);
